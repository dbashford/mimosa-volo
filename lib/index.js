"use strict";
var directories, fs, logger, path, prepArgs, register, runVolo, volo, wrench,
  __slice = [].slice,
  _this = this;
...
exports.registerCommand = function(program, retrieveConfig) {
  return register(program, function() {
    ...
    return retrieveConfig(false, function(config) {
      ...
      return program.choose(dirs, function(i) {
...
register = function(program, callback) {
  return program.command('import').description("import libraries from github via the command line using volo").option("-n, --noamd", "will load the non-amd version").option("-D, --debug", "run in debug mode").action(callback).on('--help', function() {
